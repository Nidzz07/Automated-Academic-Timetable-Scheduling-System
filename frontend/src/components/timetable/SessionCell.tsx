...
import { BookOpen, User, DoorOpen, Pin } from 'lucide-react';
...
export const SessionCell: React.FC<SessionCellProps> = ({ session, onSelect }) => {
  return (
    <div
      data-testid={`session-cell-${session.sessionId}`}
      data-session-id={session.sessionId}
      onClick={onSelect}
      className={`group relative flex flex-col justify-between h-full w-full rounded-xl border p-3 shadow-xs transition-all hover:shadow-md cursor-pointer ${
        session.isPinned
